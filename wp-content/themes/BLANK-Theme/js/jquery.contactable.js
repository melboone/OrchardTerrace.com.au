	...
	jQuery.fn.contactable = function(options) {
		...
		return this.each(function() {
			...
			*/
			...
			jQuery(this).html('<div id="contactable-inner"></div><div id="close-register"></div><form id="contactable-contactForm" method="" action=""><div id="contactable-loading"></div><div id="contactable-callback"></div><div class="contactable-holder"><p><label for="contactable-name">'+options.name+'<span class="contactable-green"> </span></label><input placeholder="NAME" id="contactable-name" class="contactable-contact contactable-validate" name="name" /></p><p><input placeholder="EMAIL" id="contactable-email" class="contactable-contact contactable-validate" name="email" /></p>'+dropdown+'<p><input placeholder="MOBILE" id="contactable-message" name="message" class="contactable-message contactable-validate" ></input></p><p><input class="contactable-submit" type="submit" value="'+options.submit+'"/></p><p class="contactable-disclaimer">'+options.disclaimer+'</p></div></form>');
			
			// Toggle the form visibility
			jQuery('#close-register').toggle(function() {
			//	jQuery('#contactable-inner').hide();	
				jQuery('#contactable-overlay').css({display: 'block'});
				jQuery(this).animate({"marginRight": "-=5px"}, "2000"); 
				jQuery('#contactable-contactForm').animate({"marginRight": "-=0px"}, "2000");
				jQuery(this).animate({"marginRight": "+=430px"}, "4000"); 
				jQuery('#contactable-contactForm').animate({"marginRight": "+=430px"}, "4000"); 
			}, 
			function() {
				jQuery('#contactable-contactForm').animate({"marginRight": "-=430px"}, "4000");
				jQuery(this).animate({"marginRight": "-=430px"}, "4000").animate({"marginRight": "+=5px"}, "2000"); 
					...
					success: function(data) {
						...
						if( data.response === 'success') {
							...
							if(options.hideOnSubmit === true) {
								...
								jQuery('#close-register').animate({dummy:1}, 2000).animate({"marginRight": "-=447px"}, "slow").animate({"marginRight": "+=5px"}, "fast");  
								jQuery('#contactable-overlay').css({display: 'none'});	
							}
						} else {
